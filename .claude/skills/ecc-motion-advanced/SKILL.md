...
description: Advanced motion patterns for React / Next.js — drag & drop, gestures, text animations, SVG path drawing, custom hooks, imperative sequences (useAnimate), loaders, and the full API decision tree. Requires ecc-motion-foundations.
version: 1.0
...
Requires `ecc-motion-foundations` to be set up first.
Use these when `ecc-motion-patterns` is not enough.
...
6. **All token values are imported from `ecc-motion-foundations`.** No inline numbers.
...
`AnimatePresence`, and tokens from `ecc-motion-foundations`:
...
- Standard UI patterns (button, modal, stagger, page transitions) → see `ecc-motion-patterns`
...
- **`ecc-motion-foundations`** — defines all tokens, springs, `useSafeMotion`, and SSR guards imported here. Must be set up before using this skill.
- **`ecc-motion-patterns`** — handles standard UI patterns (button, modal, stagger, page transitions, scroll reveals). Use it before reaching for the advanced patterns here.
