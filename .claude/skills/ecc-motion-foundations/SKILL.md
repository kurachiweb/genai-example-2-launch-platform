...
rule that downstream skills (`ecc-motion-patterns`, `ecc-motion-advanced`) inherit.
...
- UI component patterns (button, modal, stagger) → see `ecc-motion-patterns`
- Drag, gestures, SVG, text animations, custom hooks → see `ecc-motion-advanced`
...
- **`ecc-motion-patterns`** — consumes tokens and springs defined here to build button, modal, stagger, page transition, and scroll patterns. Does not redefine any values.
- **`ecc-motion-advanced`** — consumes tokens and springs defined here for drag, SVG, text, and gesture patterns. Adds `useAnimate` sequences and custom hooks on top of this foundation.
