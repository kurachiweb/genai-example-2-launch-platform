...
- **NFR-S-020**: システムは、ログインセッションの有効期限を利用者側でアイドル14日・絶対90日、管理者側でアイドル1時間・絶対12時間とし、いずれかを超過したセッションを失効させ再認証を求めなければならない(SW-004に対応)
- **NFR-S-021**: システムは、ログアウト時にサーバー側のセッションを失効させるとともに、認証Cookieを失効させなければならない(NFR-S-001、SW-004に対応)
- **NFR-S-022**: システムは、パスワード変更・多要素認証の設定変更・アカウント削除リクエスト受付のいずれかが発生した場合、当該アカウントの全端末のセッションを一括失効させなければならない
