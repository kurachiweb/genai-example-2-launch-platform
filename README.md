...
- スポンサーシップ機能(過去にローンチしたプロダクトを対価と引き換えにトップページの広告スペースへ掲載)
- ユーザー認証・認可機能
- 管理機能
...
- **FR-R-001**: システムは、勝利まで再ローンチ可能な機能を提供しなければならない。有料プランユーザーは毎日再ローンチ可とするが、他ユーザーは少なくとも直近のローンチ開始時間から72時間後でなければ再ローンチできないよう制限されなければならない
...
- **FR-SP-002**: システムは、スポンサー広告の掲載プランとして、金額が異なる複数のティア(Silver/Gold/Legend)を提供しなければならない
- **FR-SP-003**: システムは、スポンサー広告の掲載期間として、1日単位で任意の期間を選べるようにしなければならない
- **FR-SP-004**: システムは、スポンサー広告の申し込みに対し、掲載プロダクト中にプラン及び期間に応じた金額を表示し、FR-PM-001による都度決済を提供しなければならない(SW-010に対応)
...
- **NFR-A-008**: システムは、Stripeがプラットフォーム障害やアカウントBAN等の理由で利用できない場合、既存のサブスクリプション加入者の特典は維持し、新規決済フロー利用者にはフォールバックとしてCreem API及びCreem Webhookによる決済を提供しなければならない(FR-PM-001〜006、NFR-S-011に対応)
