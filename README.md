...
- **FR-U-008**: システムは、ユーザー登録時にメールアドレス確認機能を提供し、確認完了まで投稿機能やUpvoteを制限しなければならない(NFR-S-026〜028に対応)
- **FR-U-009**: システムは、ユーザーのメールアドレス確認完了時に使い捨てメールアドレスかを判定し、そのフラグを保存しなければならない(LR-003に対応)
- **FR-U-010**: システムは、メールアドレスによるパスワード再設定機能を提供しなければならない(NFR-S-026〜028に対応)
- **FR-U-011**: システムは、ユーザーが自身のログイン中セッションの一覧(端末情報・IPアドレス・最終利用日時)を確認し、個別または一括で失効させられる機能を提供しなければならない(NFR-S-021〜023に対応)
- **FR-U-012**: システムは、ユーザーが任意でTOTPによる多要素認証を有効化・無効化でき、リカバリコードを再発行できる機能を提供しなければならない(NFR-S-024、NFR-S-025に対応)
...
- **FR-A-008**: システムは、管理者が自身のTOTPによる多要素認証を登録・再設定でき、リカバリコードを再発行できる機能を提供しなければならない(NFR-S-024、NFR-S-025に対応)
...
- **FR-F-006**: システムは、署名付きURLの発行対象となるオブジェクトキー(R2内での保存パス)をサーバー側で採番し、1つの署名付きURLにつき1オブジェクトの書き込みのみを許可しなければならない(NFR-S-032に対応)
...
- **SW-006**: システムは、Cloudflare R2を画像等ファイルのオブジェクトストレージとし、署名付きURLによるR2への直接アップロードをサポートしなければならない(FR-F-004〜006、NFR-S-031、NFR-S-032に対応)
...
- **SW-009**: システムは、Cloudflare Email ServiceのEmail Sendingにより、各種通知メールの配信経路を提供しなければならない(FR-N-001〜003に対応)
...
- **SW-013**: システムは、Have I Been PwnedのPwned Passwords range API(k-Anonymity方式)によるパスワード漏洩照合を提供しなければならない(NFR-S-019に対応)
...
- **NFR-S-016**: システムは、同一ユーザーや同一管理者のアカウントに対しログイン試行が連続で失敗する場合、6回目以降の失敗時は指数バックオフの遅延(最初は1秒、失敗毎に2倍、最大5分)を挿入しなければならない
- **NFR-S-017**: システムは、ユーザーや管理者アカウントのパスワードをNFR-S-004の方式でハッシュ化して保存しなければならない
- **NFR-S-018**: システムは、ユーザーや管理者アカウントのパスワードを8文字以上128文字以下で受け付け、大文字・数字・記号等の文字種の組み合わせを必須としてはならない(NIST SP 800-63Bに準拠)
- **NFR-S-019**: システムは、パスワードの新規登録・変更・再設定時に漏洩パスワードデータベース(SW-013)と照合し、漏洩済みと判明したパスワードを拒否しなければならない
- **NFR-S-020**: システムは、ユーザーハンドル・表示名・メールアドレスのローカル部と一致するパスワードを拒否しなければならない
- **NFR-S-021**: システムは、ログインセッションの有効期限を利用者側でアイドル14日・絶対90日、管理者側でアイドル1時間・絶対12時間とし、いずれかを超過したセッションを失効させ再認証を求めなければならない(SW-003に対応)
- **NFR-S-022**: システムは、ログアウト時にサーバー側のセッションレコードを即時削除するとともに、認証Cookieを失効させなければならない(NFR-S-001、SW-003に対応)
- **NFR-S-023**: システムは、パスワード変更・多要素認証の設定変更・アカウント削除リクエスト受付のいずれかが発生した場合、当該アカウントの全端末のセッションレコードを即時に一括削除しなければならない(SW-003に対応)
- **NFR-S-024**: システムは、管理者アカウントのログインではTOTP(RFC 6238)による多要素認証を必須としなければならない。利用者アカウントについては任意とする(FR-U-012、FR-A-008に対応)
- **NFR-S-025**: システムは、多要素認証の有効化時に単回使用のリカバリコードを10個発行し、NFR-S-005の方式でハッシュ化して保存しなければならない。平文は発行時にのみ提示する
- **NFR-S-026**: システムは、パスワード再設定トークン(FR-U-010)の有効期限を発行から30分、メールアドレス確認トークン(FR-U-008)の有効期限を発行から24時間とし、いずれも1回の使用で失効させなければならない
- **NFR-S-027**: システムは、パスワード再設定トークン及びメールアドレス確認トークンをNFR-S-005の方式でハッシュ化して保存し、平文は送信するメール内のURLでのみ提示しなければならない
- **NFR-S-028**: システムは、トークンの再発行時に同一アカウント・同一用途の未使用トークンを全て失効させ、さらにパスワード変更完了時には未使用のパスワード再設定トークンを、メールアドレス変更時には未使用のメールアドレス確認トークンを全て失効させなければならない
- **NFR-S-029**: システムは、公開APIのAPIキーをNFR-S-005の方式でハッシュ化して保存し、平文は発行時にのみユーザーへ提示しなければならない(FR-PA-001に対応)
- **NFR-S-030**: システムは、公開APIへの各リクエストについて、対象機能に紐づくAPIキーの権限スコープ(FR-PA-006)に違反する操作を403 Forbiddenで拒否しなければならない
- **NFR-S-031**: システムは、R2への直接アップロード用署名付きURLの有効期限を発行から15分としなければならない(SW-006、FR-F-004に対応)
- **NFR-S-032**: システムは、R2への直接アップロード用署名付きURLに`If-None-Match: *`を署名対象ヘッダーとして含め、対象オブジェクトが既に存在する場合のアップロードをR2に412 Precondition Failedで拒否させなければならない(SW-006、FR-F-006に対応)
...
- **NFR-A-008**: システムは、Stripeがプラットフォーム障害やアカウントBAN等の理由で利用できない場合、フォールバックであるCreem API及びCreem Webhookを通じて決済処理を継続できなければならない(FR-PM-001〜006、NFR-S-011に対応)
