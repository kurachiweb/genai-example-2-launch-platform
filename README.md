...
Launch Stadiumのプログラム一式、及びドキュメント。詳細は以下のソフトウェア要件定義書を参照すること。
...
本文書は、Launch Stadiumのソフトウェア要件を定義する。本システムは、プロダクト同士の1対1対戦形式によるローンチプラットフォームを提供する。
...
- Upvoteによるプロダクト支援機能
- 勝利したプロダクトのみを掲載するディレクトリ
- トーナメント機能(Product of the Week/Year)
- 再ローンチ機能(勝利後一定期間は同じプロダクトを再投稿できない)
- プロダクトへのコメント機能
...
5. ~~再ローンチが規約で制限されているか、そうでなくてもコミュニティから忌み嫌われる~~ → 勝てるまで毎日投稿OK。勝ったら一定期間は同じプロダクトを再投稿できない。
