...
- **FR-U-002**: システムは、ユーザー登録、ログイン時、プロフィール編集、削除リクエスト時にはCloudflare Turnstileによる検証に成功した場合のみ、その処理を許可しなければならない(LR-003に対応)
...
- **FR-P-006**: システムは、プロダクト操作時にはCloudflare Turnstileによる検証に成功した場合のみ、その処理を許可しなければならない(LR-003に対応)
...
- **SW-012**: システムは、Cloudflare Turnstileによるウィジェット埋め込み及びAPIサーバー側でのsiteverify検証機能を提供しなければならない(FR-U-002、FR-P-006、FR-S-005、FR-RP-004に対応)
