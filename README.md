...
- プロダクトへのコメント機能
- 公開APIを通じた外部連携機能(APIキー認証・レート制限付き)
- 将来的な有料プラン・決済機能(Creemベース、提供有無・内容は別途確定)
...
- **FR-PA-002**: システムは、公開APIへの全リクエストに対し、APIキーによる認証を要求しなければならない
- **FR-PA-003**: システムは、公開APIについて1分あたりのリクエスト数制限(レート制限)を実施し、超過時は429 Too Many Requestsを返却しなければならない
- **FR-PA-004**: システムは、公開APIを通じて、自身のプロフィール・投稿製品・コメントのCRUD操作を提供しなければならない
- **FR-PA-005**: システムは、公開APIのドキュメントをSwagger UI(OpenAPI形式)で提供しなければならない(NFR-M-007に対応)
- **FR-PA-006**: システムは、ユーザーが自身のAPIキーを失効・再発行できる機能を提供しなければならない

#### 4.12 決済・有料プラン機能

有料プランの提供有無及び具体的な内容(何を有料化するか)は本文書作成時点で未確定であり、別途ビジネス側で確定した上で本節を改訂する。以下は、将来的に有料プランを提供する場合に最低限満たすべき条件付き要件である。

- **FR-PM-001**: システムは、有料プランを提供する場合、Creem Checkoutを通じた決済ページへの誘導機能を提供しなければならない
- **FR-PM-002**: システムは、有料プランを提供する場合、Creem Webhookにより都度支払い・定期課金イベントを受信し、ユーザーのプラン状態へ反映しなければならない
- **FR-PM-003**: システムは、有料プランを提供する場合、ユーザーが自身の決済履歴及び現在のプラン状態を確認できる機能を提供しなければならない
...
- **NFR-S-004**: システムは、Node.js標準のcrypto.subtleによりパスワードをPBKDF2-HMAC-SHA512形式でハッシュ化しなければならない
...
- **NFR-A-007**: システムは、Cloudflare D1 Time Travelによるポイントインタイムリストア(過去30日以内のみ対応)を第一の復旧手段とし、30日を超える障害対応や誤削除に対する恒久的なデータ保全が必要な場合に備え、定期的なエクスポート等の補完的バックアップ手段を講じなければならない
