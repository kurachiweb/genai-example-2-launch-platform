...
- **再ローンチ**: マッチに敗北したプロダクトを、勝利するまで再度ローンチすること。有料プラン未加入ユーザーは間隔・回数を制限される(4.7節)
- **有料プラン**: 都度決済の免除及び再ローンチの間隔・回数制限解除等の特典を付与する定期課金プラン
- **APIキー**: 公開API利用時に発行される認証用の鍵。全リクエストに対する認証に用いる
...
- **FR-PA-002**: システムは、公開APIが提供するエンドポイントへの全リクエストに対し、APIキーによる認証を要求しなければならない。ただしOpenAPI仕様書(エンドポイント一覧)配信用のエンドポイントは、APIドキュメント提供ツールが参照するため認証の対象外とする
...
- **SW-008**: システムは、Amazon SESとのSMTP連携により、各種通知メールの配信経路を提供しなければならない(FR-N-001〜003に対応)
