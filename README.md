...
- **FR-U-005**: システムは、ユーザープロフィール画像のアップロード機能を提供しなければならない(FR-F-001〜004に対応)
...
- **FR-U-008**: システムは、ユーザー登録時にメールアドレス確認機能を提供し、確認完了まで投稿機能やUpvoteを制限しなければならない
- **FR-U-009**: システムは、ユーザーのメールアドレス確認完了時に使い捨てメールアドレスかを判定し、そのフラグを保存しなければならない(LR-003に対応)
- **FR-U-010**: システムは、メールアドレスによるパスワード再設定機能を提供しなければならない
- **FR-U-011**: システムは、削除リクエスト受付後、DR-002に定める猶予期間内に個人データを完全に削除しなければならない

#### 4.2 プロダクト管理機能

- **FR-P-001**: システムは、プロダクト登録機能を提供しなければならない
- **FR-P-002**: システムは、マークダウン形式によるプロダクト説明入力を受け付けなければならない
- **FR-P-003**: システムは、プロダクト画像のアップロード機能を提供しなければならない(FR-F-001〜004に対応)
...
- **FR-F-004**: システムは、アップロードされたファイルがサポート対象外形式あるいはファイルサイズ超過である場合、R2から削除しなければならない(SW-007に対応)
...
- **UI-006**: システムは、ファイルアップロード時にプレビュー表示(画像の場合は編集ダイアログによるリサイズ・切り取りを含む)、ドラッグ&ドロップ、ファイル形式・サイズ・枚数制限の事前フィードバックを提供しなければならない(FR-F-001〜004に対応)
...
- **SW-006**: システムは、Cloudflare R2を画像等ファイルのオブジェクトストレージとし、署名付きURLによるR2への直接アップロードをサポートしなければならない
...
- **NFR-S-015**: システムは、同一ユーザーや同一管理者のアカウントに対しログイン試行が5回連続失敗した場合、15分間ログイン処理を拒否しなければならない
- **NFR-S-016**: システムは、ユーザーや管理者アカウントのパスワードをNFR-S-004の方式でハッシュ化して保存しなければならない
- **NFR-S-017**: システムは、公開APIのAPIキーをNFR-S-004の方式でハッシュ化して保存し、平文は発行時にのみユーザーへ提示しなければならない(FR-PA-001に対応)
- **NFR-S-018**: システムは、公開APIへの各リクエストについて、対象機能に紐づくAPIキーの権限スコープ(FR-PA-006)に違反する操作を403 Forbiddenで拒否しなければならない
