...
- **SW-007**: システムは、Cloudflare R2を画像・ファイルのオブジェクトストレージとし、署名付きURLによるR2への直接アップロードをサポートしなければならない
...
- **SW-011**: システムは、Cloudflare Turnstileによるウィジェット埋め込み及びAPIサーバー側でのsiteverify検証機能を提供しなければならない(FR-U-002、FR-P-006、FR-S-005、FR-RP-004に対応)
