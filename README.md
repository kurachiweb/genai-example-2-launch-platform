...
- **NFR-S-015**: システムは、同一アカウントに対しログイン試行が5回連続失敗した場合、15分間ログイン処理を拒否しなければならない
- **NFR-S-016**: システムは、公開APIのAPIキーをNFR-S-004と同一の方式でハッシュ化して保存し、平文は発行時にのみユーザーへ提示しなければならない(FR-PA-001に対応)
- **NFR-S-017**: システムは、公開APIへの各リクエストについて、対象機能に紐づくAPIキーの権限スコープ(FR-PA-006)に違反する操作を403 Forbiddenで拒否しなければならない
