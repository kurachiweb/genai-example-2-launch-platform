...
- **FR-M-005**: システムは、UTC時間8時に対戦開始、翌7時55分に対戦終了するよう制御しなければならない
...
- **FR-N-002**: システムは、ローンチ開始時間の5分前に対戦開始予告通知機能を提供しなければならない
- **FR-N-003**: システムは、ローンチ終了時間の後に勝敗結果通知機能を提供しなければならない
- **FR-N-004**: システムは、ユーザーが通知設定画面において、FR-N-001〜003の通知メールそれぞれについて受信可否を種類ごとに個別設定できる機能を提供しなければならない(LR-005に対応)
- **FR-N-005**: システムは、送信する全ての通知メールに、送信者情報(事業者名及び連絡先)並びに当該通知種別の配信停止用リンクを表示しなければならない(LR-005に対応)
- **FR-N-006**: システムは、配信停止用リンクについて、ログイン不要かつ1クリックの操作のみで当該通知種別の配信停止が完了する機能を提供しなければならない(FR-N-005に対応)
- **FR-N-007**: システムは、配信停止用リンクのトークンをユーザー及び通知種別ごとに発行し、NFR-S-005の方式でハッシュ化して保存し、平文は送信するメール内のURLでのみ提示しなければならない
- **FR-N-008**: システムは、配信停止用リンクのトークンについて、ユーザーが退会するまでの間有効としなければならない
...
- **SW-009**: システムは、Cloudflare Email ServiceのEmail Sendingにより、各種通知メールの配信経路を提供しなければならない(FR-N-001〜008に対応)
