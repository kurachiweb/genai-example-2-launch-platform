...
このドキュメントは`.claude/rules`配下や`.claude/skills`配下のスキル・コマンド・ルール・エージェント定義ファイルについて、概要・呼び出しトリガー・使用目的を説明する。
...
**概要**: システム設計・スケーラビリティ・技術的意思決定の専門家。
...
**概要**: React/JSX専門のシニアコードレビュアー。フック正当性・レンダリングパフォーマンス・サーバー/クライアント境界・アクセシビリティ・React固有セキュリティを審査する。
...
TanStack Start、及びTanStack Router・TanStack Queryのベストプラクティス集。各スキルは`rules/`配下に個別ルールファイルを持つ。

| スキル                                       | トリガー                                              | 概要                                                                                                                                                                                                                         |
| -------------------------------------------- | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tanstack-agent-skills:tanstack-start`       | TanStack Startでフルスタックアプリを構築する時        | サーバー関数(`createServerFn`)・ミドルウェア・SSR・認証・デプロイパターンのベストプラクティス集。CRITICAL(サーバー関数・セキュリティ)からLOW(ファイル構成・デプロイアダプター)までの優先度別ルール表を提供                   |
| `tanstack-agent-skills:tanstack-router`      | 複雑なルーティング要件を持つReactアプリを構築する時   | 型安全なルーティング・データローディング・検索パラメータ・ナビゲーションのベストプラクティス集。ファイルベースルーティング・ルートローダー・`beforeLoad`によるコンテキスト注入・intentベースのプリロードなどを優先度別に整理 |
| `tanstack-agent-skills:tanstack-query`       | サーバー状態を扱うデータ駆動Reactアプリを構築する時   | データフェッチング・キャッシュ・ミューテーション・サーバー状態同期のベストプラクティス集。クエリキー設計・`staleTime`/`gcTime`設定・楽観的更新・エラーバウンダリ・SSRハイドレーションなどを優先度別に整理                    |
| `tanstack-agent-skills:tanstack-integration` | TanStack Query・Router・Startを組み合わせて統合する時 | 3ライブラリ統合時のセットアップ・データフロー・キャッシュ協調・SSR統合パターン集。`setupRouterSsrQueryIntegration`によるRouter+QueryのSSR自動連携、ローダーでの`ensureQueryData`活用などを扱う                               |
