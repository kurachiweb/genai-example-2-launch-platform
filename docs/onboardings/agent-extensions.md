...
| スキル                 | トリガー                                                             | 概要                                                                                                                                                                                        |
| ----------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cc-sdd:kiro-discovery`       | 新しい作業依頼のエントリーポイント（`/kiro-discovery "アイデア"`）  | 既存 spec の状態を軽量スキャンし、既存 spec 拡張・spec 不要・新規単一 spec・複数 spec への分解・混在分解のいずれかの対応方針を判定。対話でアイデアを深掘りし、`brief.md`／`roadmap.md` をディスクに書き出してから次のコマンドを提案する |
...
| スキル                   | トリガー                                                                    | 概要                                                                                                                                                                                       |
| ------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cc-sdd:kiro-impl`             | 承認済みタスクの実装時（`/kiro-impl {feature} [task番号]`）                     | タスク番号なし＝自律モード（タスクごとに新規サブエージェントを起動し実装→独立レビュー→コミットを繰り返す）、番号指定＝手動モード（メインコンテキストで TDD サイクルを実行）。実装は Red-Green-Refactor を厳守し、行き詰まった場合は `cc-sdd:kiro-debug` に委譲する |
